...
let songIndex = 0;
const songPaths = ["/audio/song/REPO.mp3", "/audio/song/Bob.mp3"];
let musicInstances = []; 

// Optimization: Music only preloads after Enter
function loadMusic() {
...
// --- LOADERS ---
const textureLoader = new THREE.TextureLoader(loadingManager); // FIX: Added Manager
const dracoloader = new DRACOLoader();
dracoloader.setDecoderPath('/draco/');
const loader = new GLTFLoader(loadingManager);
loader.setDRACOLoader(dracoloader);

const skybox = new THREE.CubeTextureLoader(loadingManager) // FIX: Added Manager
...
Object.entries(Textures).forEach(([key, path]) => {
  ...
  loadedTexture.night[key] = dayTexture; 
});
...
if(enterBtn) {
  enterBtn.addEventListener("click", () => {
    ...
    // Lazy Load night textures & music & video
    loadMusic();
    loadVideo();
    loadNightTextures();
...
function loadNightTextures() {
  ...
  Object.entries(Textures).forEach(([key, path]) => {
    nightLoader.load(path.night, (tex) => {
      tex.flipY = false;
      tex.colorSpace = THREE.SRGBColorSpace;
      loadedTexture.night[key] = tex;
    });
  });
}
...
function loadVideo() {
  ...
  ScreenVideo.play().then(() => {
    ...
    if(screenMesh) {
      ...
      gsap.to(screenMesh.material.color, { r: 1, g: 1, b: 1, duration: 2 });
    }
  });
}
...
// --- Models ---
let chest, boombox;
const ropes = [], pointers = [], figurines = [], plants = [], dragons = [];
    ...
    if (child.isMesh) {
      ...
      Object.keys(Textures).forEach((key) => {
        if (child.name.includes(key)) {
          child.material = new THREE.MeshBasicMaterial({ map: loadedTexture.day[key] });
...
const render = () => {
  ...
  if (intersects.length > 0) {
    ...
    if (intersected.name.includes("Target") || intersected.name.includes("Hover") || intersected.name.includes("Pointer") || intersected.name.includes("Pannel")) {
